...
function optimizeMaxSharpe(assets: Asset[]): number[] {
  ...
  for (let iter = 0; iter < iterations; iter++) {
    ...
    const vol = calculatePortfolioVolatility(assets, weights);
  ...
  for (let iter = 0; iter < gradientIterations; iter++) {
    ...
    const currentVol = calculatePortfolioVolatility(assets, currentWeights);
    ...
    for (let i = 0; i < n; i++) {
      for (let j = i + 1; j < n; j++) {
        ...
        const testVol = calculatePortfolioVolatility(assets, normalized);
...
function optimizeMaxConvexity(assets: Asset[]): number[] {
  const n = assets.length;
  if (n === 1) return [1];
...
export function optimizePortfolio(
  ...
  switch (goal) {
    case "max_return":
      ...
      break;
    case "max_sharpe":
      optimizedWeights = optimizeMaxSharpe(assets);
      ...
      break;
    case "max_convexity":
      optimizedWeights = optimizeMaxConvexity(assets);
  ...
  const portfolioVolatility = calculatePortfolioVolatility(assets, optimizedWeights);
